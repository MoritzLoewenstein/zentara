...
function createDashboardContent() {
	return {
		...
		deleteBookmarkGroup(group_index) {
			dashboard_content_state.dashboard_edit.bookmarkGroups.splice(
				group_index,
				1,
			);
		},
		...
		setBookmarkMove(group_index, bookmark_index) {
			...
			dashboard_content.value.dashboard_edit.bookmarkGroups[
				group_index
			].bookmarks[bookmark_index].moving = true;
		},
		getBookmarkMove() {
			...
			const bookmark =
				dashboard_content_state.dashboard_edit.bookmarkGroups[move_group_index]
					.bookmarks[move_bookmark_index];
			return bookmark;
		},
		resetBookmarkMove() {
			const group_index = dashboard_content_state.bookmark_move.group_index;
			const bookmark_index =
				dashboard_content_state.bookmark_move.bookmark_index;
			...
			dashboard_content_state.dashboard_edit.bookmarkGroups[
				group_index
			].bookmarks[bookmark_index].moving = undefined;
			dashboard_content_state.bookmark_move = {
				group_index: null,
				bookmark_index: null,
			};
		...
		updateBookmarkMovePreview(group_index, bookmark_index) {
			...
			dashboard_content_state.dashboard_edit.bookmarkGroups[
				group_index
			].bookmarks.splice(bookmark_index, 0, { ...bookmark, movePreview: true });
		},
		resetBookmarkMovePreview() {
			// remove all bookmark move previews
			dashboard_content_state.dashboard_edit.bookmarkGroups.forEach(
				(group, index) => {
					dashboard_content_state.dashboard_edit.bookmarkGroups[
						index
					].bookmarks = group.bookmarks.filter(
						(bookmark) => !bookmark.movePreview,
					);
				},
			);
		},
		moveBookmark(
			...
			dashboard_content_state.dashboard_edit.bookmarkGroups[
				move_group_index
			].bookmarks.splice(move_bookmark_index, 1);
			//set bookmark details
			bookmark.moving = undefined;
			// insert bookmark at new position
			dashboard_content_state.dashboard_edit.bookmarkGroups[
				target_group_index
			].bookmarks.splice(target_bookmark_index, 0, { ...bookmark });
		},
		...
		deleteApplicationGroup(group_index) {
			dashboard_content_state.dashboard_edit.applicationGroups.splice(
				group_index,
				1,
			);
		},
